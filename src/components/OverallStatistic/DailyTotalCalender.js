...
const DailyTotalCalender = (props) => {
  ...
  const isMonthly = props.isMonthly;
  ...
  if (expenses && currentUser && selectedMonth && selectedYear) {
    ...
    const allDatesInSelectedMonth = utils.getAllTheDatesInAMonth(
      selectedYear,
      selectedMonth
    );
    const TotalInThatDay = allDatesInSelectedMonth.map((date) => {
      let expensesOnThatDate = usersExpensesInSelectedMonthAndYear.filter(
        (exp) => Number(exp.value.date === date)
      );
      totals[date] =
        expensesOnThatDate.map((elem) => Number(elem.value.expense)).length >= 1
          ? expensesOnThatDate
              .map((elem) => Number(elem.value.expense))
    ...
    for (let i = 0; i < moment(allDatesInSelectedMonth[0]).day(); i++) {
      previDays.push(
        moment(allDatesInSelectedMonth[0])
          .subtract(i + 1, "d")
          .format("MM/DD/YYYY")
      );
      dayStart.push(i);
    ...
    for (
      ...
      Math.abs(
        moment(
          allDatesInSelectedMonth[allDatesInSelectedMonth.length - 1]
        ).day() - 6
      );
      i++
    ) {
      nextiDays.push(
        moment(allDatesInSelectedMonth[allDatesInSelectedMonth.length - 1])
          .add(i + 1, "d")
          .format("MM/DD/YYYY")
      );
      dayEnd.push(i);
    ...
    let printHtml = Object.keys(totals).map((elem, i) => {
      var listEle = (
        <li
          ...
          key={i}
          style={elem === selectedDate && !isMonthly ? listStyleToday : listStyleDateCells}
        >
          <div style={elem === selectedDate && !isMonthly? todayDateArea : dateArea}>
            {" "}
            {moment(allDatesInSelectedMonth[i]).date()}
          </div>{" "}
          {totals[elem].toFixed(2).toString().replace(/(\d)(?=(\d\d)+\d$)/g, "$1,")}{" "}
        </li>
      );
